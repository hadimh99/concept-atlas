      ...
      <main ref={containerRef} className="relative w-full flex-grow flex items-center justify-center h-screen">
        ...
        <AnimatePresence>
          {activeTab === 'search' && loading && (
            <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 2, filter: "blur(20px)" }} className="absolute z-20 flex flex-col items-center justify-center">
                  ...
                  return (
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center pointer-events-auto p-4 sm:p-0">
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={`relative w-full sm:w-[90vw] max-w-[700px] h-[85vh] flex flex-col shadow-2xl rounded-2xl z-[1001] border ${isKeyword ? 'bg-white dark:bg-slate-900 border-slate-200' : 'bg-slate-50 dark:bg-slate-900 border-slate-200'}`}>
                        <div className={`flex justify-between items-center backdrop-blur-md pt-5 pb-4 px-4 sm:px-6 z-10 border-b rounded-t-2xl shrink-0 ${isKeyword ? 'bg-slate-50/95 dark:bg-slate-900/95 border-slate-200' : 'bg-slate-50/90 dark:bg-slate-900/90 border-slate-200'}`}>
                          <h2 className={`text-lg sm:text-xl md:text-2xl font-mono font-bold tracking-tight truncate pr-4 ${isKeyword ? 'text-blue-600' : 'text-indigo-600'}`}>{data.clusters[activeCluster].theme_label}</h2>
                          <button onClick={() => setActiveCluster(null)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-full transition-colors cursor-pointer shrink-0"><X className="w-5 h-5" /></button>
                        </div>
                        <div className={`px-4 sm:px-6 py-3 border-b shrink-0 flex flex-wrap gap-2 items-center ${isKeyword ? 'bg-slate-100/50 dark:bg-slate-800/30' : 'bg-slate-200/30 dark:bg-slate-800/30'}`}>
                          <div className="flex items-center gap-1.5 text-slate-500 mr-1"><Filter className="w-3.5 h-3.5" /><span className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider">Length:</span></div>
                          {['All', 'Short', 'Medium', 'Long'].map(f => (<button key={f} onClick={() => { setLengthFilter(f); setCurrentPage(1); if (modalScrollRef.current) modalScrollRef.current.scrollTop = 0; }} className={`px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-medium transition-colors cursor-pointer ${lengthFilter === f ? (isKeyword ? 'bg-blue-500 text-white' : 'bg-indigo-500 text-white') : 'bg-white dark:bg-slate-800 text-slate-600 border border-slate-200'}`}>{f}</button>))}
